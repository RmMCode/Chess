...
/* ----------------------------------------------------------------------- */
/* Fixed Task 3 here - Both players auto complete - By Rishit Maiti*/
play(Board) :-
    % WHITE (playerA) moves automatically
    execute_command(playerA, Board, BoardA),
    
    % print AFTER white’s move
    nl, write('White moved.'), nl,
    print_board(BoardA),

    % BLACK (playerB) moves automatically
    execute_command(playerB, BoardA, BoardB),

    % print AFTER black’s move
    nl, write('Black moved.'), nl,
    print_board(BoardB),
...
/* ----------------------------------------------------------------------- */
% ---------- Strength Function ----------
strengthA([state(_, _, _, _)|Board], Color, OppColor, Strength) :-
    strengthA(Board, Color, OppColor, Strength), !.
strengthA([piece(_, Color, Type)|Board], Color, OppColor, Strength) :-
    valueA(Type, Value),
    strengthA(Board, Color, OppColor, Partial),
    Strength is Partial + Value, !.
strengthA([piece(_, OppColor, Type)|Board], Color, OppColor, Strength) :-
    valueA(Type, Value),
    strengthA(Board, Color, OppColor, Partial),
    Strength is Partial - Value.
strengthA([], _, _, 0).

ply_depthA(3).     % same as playerB’s depth
...
% White opening e2–e4
bookA([
    state(white, WK, WRK, WQR),
    state(black, BK, BRK, BQR),
    piece(a-8, black, rook), piece(b-8, black, knight),
    piece(c-8, black, bishop), piece(d-8, black, queen),
    piece(e-8, black, king), piece(f-8, black, bishop),
    piece(g-8, black, knight), piece(h-8, black, rook),
    ...
    piece(h-2, white, pawn), piece(e-2, white, pawn)
], e-2, e-4).

% ---------- Move Collection ----------
collect_movesA(Board, Color, Moves) :-
    bagof(move(F, T), P^move(Board, F, T, Color, P), Moves).

% ---------- Alpha–Beta Pruning Helpers ----------
sufficientA(Player, Board, Turn, [], D, A, B, M, V, M, V) :- !.
sufficientA(Player, Board, Turn, Moves, D, A, B, M, V, M, V) :-
    Player \== Turn, V < A, !.
sufficientA(Player, Board, Turn, Moves, D, A, B, M, V, M, V) :-
    Player = Turn, V > B, !.
sufficientA(Player, Board, Turn, Moves, D, A, B, M, V, BestM, BestV) :-
    new_bounds(Player, Turn, A, B, V, NA, NB),
    find_best(Player, Board, Turn, Moves, D, NA, NB, M1, V1),
    better_of(Player, Turn, M, V, M1, V1, BestM, BestV).
...
drawCell(BoardStates, Row, Col) :-
  pair(Name, Col),
  % checks whether or not there is a piece at that cell
  myMember(piece(Name-Row, Color, Piece), BoardStates),
  drawSymbol(' ', 1),
  (
    % for black pieces, draw '*', for white pieces, draw ' '
...
drawCell(BoardStates, Row, Col) :-
  pair(Name, Col),
  % checks whether or not there is a piece at that cell
  % \+ is a not operator in prolog, whcih is used here to check if there is no piece at that cell
  \+ (myMember(piece(Name-Row, Color, Piece), BoardStates)),
  drawSymbol(' ', 4).
